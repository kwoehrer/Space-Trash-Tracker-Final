...
class HTTP {
    constructor(llparse, mode = 'loose') {
        this.llparse = llparse;
        this.mode = mode;
        this.nodes = new Map();
        const p = llparse;
        this.url = new url_1.URL(p, mode);
        this.TOKEN = mode === 'strict' ? constants_1.STRICT_TOKEN : constants_1.TOKEN;
        this.span = {
    ...
    build() {
        ...
        // Verify defaults
        assert.strictEqual(constants_1.FINISH.SAFE, 0);
        assert.strictEqual(constants_1.TYPE.BOTH, 0);
    ...
    buildLine() {
        ...
        n('start')
            .match(['\r', '\n'], n('start'))
            .otherwise(this.update('finish', constants_1.FINISH.UNSAFE, this.invokePausable('on_message_begin', constants_1.ERROR.CB_MESSAGE_BEGIN, switchType)));
        n('start_req_or_res')
            .peek('H', n('req_or_res_method'))
            .otherwise(this.update('type', constants_1.TYPE.REQUEST, 'start_req'));
        n('req_or_res_method')
            .select(constants_1.H_METHOD_MAP, this.store('method', this.update('type', constants_1.TYPE.REQUEST, 'req_first_space_before_url')))
            .match('HTTP/', this.update('type', constants_1.TYPE.RESPONSE, 'res_http_major'))
            .otherwise(p.error(constants_1.ERROR.INVALID_CONSTANT, 'Invalid word encountered'));
        // Response
        n('start_res')
            .match('HTTP/', n('res_http_major'))
            .otherwise(p.error(constants_1.ERROR.INVALID_CONSTANT, 'Expected HTTP/'));
        n('res_http_major')
            .select(constants_1.MAJOR, this.store('http_major', 'res_http_dot'))
            .otherwise(p.error(constants_1.ERROR.INVALID_VERSION, 'Invalid major version'));
        n('res_http_dot')
            .match('.', n('res_http_minor'))
            .otherwise(p.error(constants_1.ERROR.INVALID_VERSION, 'Expected dot'));
        n('res_http_minor')
            .select(constants_1.MINOR, this.store('http_minor', 'res_http_end'))
            .otherwise(p.error(constants_1.ERROR.INVALID_VERSION, 'Invalid minor version'));
        n('res_http_end')
        ...
        n('res_status_code')
            ...
            overflow: p.error(constants_1.ERROR.INVALID_STATUS, 'Response overflow'),
        ...
        if (this.mode === 'strict') {
            n('res_line_almost_done')
                .match('\n', n('header_field_start'))
                .otherwise(p.error(constants_1.ERROR.STRICT, 'Expected LF after CR'));
        }
        else {
            n('res_line_almost_done')
        ...
        // Request
        n('start_req')
            .select(constants_1.METHOD_MAP, this.store('method', 'req_first_space_before_url'))
        ...
        url.exit.toHTTP09
            .otherwise(this.update('http_major', 0, this.update('http_minor', 9, 'header_field_start')));
        const isSource = this.isEqual('method', constants_1.METHODS.SOURCE, {
            equal: n('req_http_major'),
            notEqual: p.error(constants_1.ERROR.INVALID_CONSTANT, 'Expected SOURCE method for ICE/x.x request'),
        });
        n('req_http_start')
        ...
        n('req_http_dot')
            ...
            .otherwise(p.error(constants_1.ERROR.INVALID_VERSION, 'Expected dot'));
        n('req_http_minor')
            .select(constants_1.MINOR, this.store('http_minor', 'req_http_end'))
            .otherwise(p.error(constants_1.ERROR.INVALID_VERSION, 'Invalid minor version'));
        n('req_http_end')
            .match(['\r\n', '\n'], n('header_field_start'))
    ...
    buildHeaderValue() {
        ...
        if (this.mode === 'strict') {
            ...
            n('header_value_discard_ws_almost_done').skipTo(n('header_value_discard_lws'));
        }
        n('header_value_discard_lws')
            .match([' ', '\t'], n('header_value_discard_ws'))
            .otherwise(this.setHeaderFlags(this.emptySpan(span.headerValue, 'header_field_start')));
        n('header_value_start')
            ...
            [constants_1.HEADER_STATE.TRANSFER_ENCODING]: n('header_value_te_chunked'),
        ...
        n('header_value_te_chunked')
            .transform(p.transform.toLowerUnsafe())
            .match('chunked', this.update('header_state', constants_1.HEADER_STATE.TRANSFER_ENCODING_CHUNKED, 'header_value_discard_rws'))
        ...
        n('header_value_content_length_ws')
            ...
            .peek(['\r', '\n'], this.setFlag(constants_1.FLAGS.CONTENT_LENGTH, 'header_value_discard_rws'))
        ...
        n('header_value_otherwise')
            ...
            .otherwise(p.error(constants_1.ERROR.INVALID_HEADER_TOKEN, 'Invalid header value char'));
        ...
        if (this.mode === 'strict') {
            n('headers_almost_done')
                .match('\n', checkTrailing)
                .otherwise(p.error(constants_1.ERROR.STRICT, 'Expected LF after headers'));
        }
        else {
            n('headers_almost_done')
        ...
        const ENCODING_CONFLICT = constants_1.FLAGS.CHUNKED | constants_1.FLAGS.CONTENT_LENGTH;
        const checkEncConflict = this.testFlags(ENCODING_CONFLICT, {
            1: p.error(constants_1.ERROR.UNEXPECTED_CONTENT_LENGTH, 'Content-Length can\'t be present with chunked encoding'),
        });
        ...
        n('body_identity_eof')
            .otherwise(span.body.start(this.update('finish', constants_1.FINISH.SAFE_WITH_CB, 'eof')));
        ...
        n('chunk_size')
            .select(constants_1.HEX_MAP, this.mulAdd('content_length', {
        ...
        if (this.mode === 'strict') {
            n('chunk_data_almost_done')
                .match('\r\n', n('chunk_complete'))
                .otherwise(p.error(constants_1.ERROR.STRICT, 'Expected CRLF after chunk'));
        }
        else {
            n('chunk_data_almost_done')
        ...
        n('closed')
            .match(['\r', '\n'], n('closed'))
            .skipTo(p.error(constants_1.ERROR.CLOSED_CONNECTION, 'Data after `Connection: close`'));
        n('restart')
            .otherwise(n('start'));
    }
    node(name) {
