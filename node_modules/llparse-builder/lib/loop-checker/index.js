...
const EMPTY_VALUE = new lattice_1.Lattice('empty');
const ANY_VALUE = new lattice_1.Lattice('any');
/**
...
class LoopChecker {
    ...
    propagate(node, changed) {
        ...
        for (const edge of node.getAllEdges()) {
            ...
            if (edge.key === undefined || typeof edge.key === 'number') {
                ...
                // `.peek()`
                const edgeValue = new lattice_1.Lattice([edge.key[0]]).intersect(value);
        ...
        for (const [child, childValue] of keysByTarget) {
            debug('node %j propagates %j to %j', node.name, childValue, child.name);
            this.update(child, childValue, changed);
        }
    }
    update(node, newValue, changed) {
    ...
    terminate(node, value, changed) {
        ...
        const result = new lattice_1.Lattice(terminated);
    ...
    visit(node, path) {
        ...
        for (const edge of node.getAllEdges()) {
            ...
            if (edge.key === undefined || typeof edge.key === 'number') {
                ...
                edgeValue = edgeValue.intersect(new lattice_1.Lattice([edge.key[0]]));
            }
            ...
            if (path.indexOf(edge.node) !== -1) {
                if (path.length === 0) {
                    throw new Error(`Detected loop in "${edge.node.name}" through "${edge.node.name}"`);
                }
                throw new Error(`Detected loop in "${edge.node.name}" through chain ` +
...
exports.LoopChecker = LoopChecker;
