...
class MulAdd extends field_1.Field {
    doBuild(ctx, bb) {
        ...
        const product = bb.extractvalue(mul, mul.ty.toStruct().lookupField('result').index);
        const overflowBit = bb.extractvalue(mul, mul.ty.toStruct().lookupField('overflow').index);
        const { onTrue: isOverflow, onFalse: normal } = ctx.branch(bb, overflowBit, { onTrue: 'unlikely', onFalse: 'likely' });
        ...
        const result = normal.extractvalue(add, add.ty.toStruct().lookupField('result').index);
        const addOverflowBit = normal.extractvalue(add, add.ty.toStruct().lookupField('overflow').index);
        const { onTrue: isAddOverflow, onFalse: check } = ctx.branch(normal, addOverflowBit, { onTrue: 'unlikely', onFalse: 'likely' });
        // Addition overflow
        ...
        if (options.max !== undefined) {
            ...
            const { onTrue: maxOverflow, onFalse: noMaxOverflow } = ctx.branch(check, cmp, { onTrue: 'unlikely', onFalse: 'likely' });
    ...
    buildIntrinsics(ctx, ty) {
        ...
        overRet.addField(constants_1.BOOL, 'overflow');
        overRet.finalize();
        const overSig = ctx.ir.signature(overRet, [ty, ty]);
        const postfix = `with.overflow.${ty.typeString}`;
        return {
            add: ctx.declareFunction(overSig, `llvm.${signed ? 'sadd' : 'uadd'}.${postfix}`),
            mul: ctx.declareFunction(overSig, `llvm.${signed ? 'smul' : 'umul'}.${postfix}`),
        };
    }
}
