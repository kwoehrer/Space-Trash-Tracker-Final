...
class Verify extends base_1.Pass {
    constructor() {
        super(...arguments);
    ...
    run() {
        ...
        for (const decl of this.input.declarations) {
            if (this.globals.has(decl.name)) {
                assert(decl.ty.isEqual(this.globals.get(decl.name).ty), `Conflicting type for declaration with name: "${decl.name}"`);
                continue;
            }
        ...
        for (const global of this.input.globals) {
            if (this.globals.has(global.name)) {
                assert(global.ty.isEqual(this.globals.get(global.name).ty), `Conflicting type for global with name: "${global.name}"`);
                continue;
            }
        ...
        for (const fn of this.input.functions) {
            if (this.globals.has(fn.name)) {
                assert(fn.ty.isEqual(this.globals.get(fn.name).ty), `Conflicting type for function with name: "${fn.name}"`);
                continue;
            }
    ...
    propagateReachable(reachable, bb) {
        let changed = false;
    ...
    propagateBlockLiveness(live, reachable, bb) {
        let changed = false;
        ...
        for (const i of bb) {
            if (i instanceof instructions.Phi) {
                ...
                for (const edge of i.edges) {
                    assert(preds.delete(edge.fromBlock), 'Duplicate or unknown `fromBlock`: ' +
                    ...
                    assert(livePred.has(edge.value), `Incoming value "${edge.value}" doesn't dominate phi "${i}"`);
                }
                assert.strictEqual(preds.size, 0, `phi "${i}" doesn't cover all predecessors of block "${bb}"`);
                continue;
            }
            for (const operand of i) {
                if (operand instanceof constants.Declaration) {
                    assert(this.globals.has(operand.name), `Found reference to undeclared function: "${operand}"`);
                    assert(this.globals.get(operand.name).ty.isEqual(operand.ty), `Found conflicting reference to function: "${operand}"`);
                }
                else if (operand instanceof values.Global) {
                    assert(this.globals.has(operand.name), `Found reference to undeclared global: "${operand}"`);
                    assert(this.globals.get(operand.name).ty.isEqual(operand.ty), `Found conflicting reference to global: "${operand}"`);
                }
                else if (operand instanceof instructions.Instruction) {
                    assert(liveSet.has(operand), `Declaration of "${operand}" ` +
